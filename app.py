...
def add_business(business_name, client_email, state, license_type, last_renewal, next_renewal):
...
with st.sidebar:
    ...
    client_email = st.text_input("Client Email", placeholder="e.g. owner@rodrigueselectrical.com")
    ...
    if st.button("Add to Tracker", type="primary", use_container_width=True):
        if business_name and client_email:
            next_date = calculate_next_renewal(state, license_type, last_renewal, RULES)
            add_business(business_name, client_email, state, license_type, last_renewal, next_date)
            st.success(f"Added! Next renewal: {next_date.strftime('%B %d, %Y')}")
            st.rerun()
        else:
            st.error("Please enter both a business name and client email.")
